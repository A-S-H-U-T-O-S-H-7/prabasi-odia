// src/components/admin/associates/CreateAssociateModal.tsx
'use client';
...
interface CreateAssociateModalProps {
  ...
  editingAssociate?: Associate | null;
  isSaving?: boolean;
}
...
export default function CreateAssociateModal({
  ...
  useEffect(() => {
    if (editingAssociate) {
      setFormData({
        ...
        logo: editingAssociate.logo || "",
        logoFile: null,
        logoPreview: editingAssociate.logo || "",
        website: editingAssociate.website || "",
        isActive: editingAssociate.isActive !== undefined ? editingAssociate.isActive : true,
        displayOrder: editingAssociate.displayOrder || 0,
      });
    } else {
      setFormData({
        name: "",
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) newErrors.name = "Associate name is required";
      ...
      <div className="rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col bg-white/95 backdrop-blur-sm border border-[#E7D7E8] shadow-2xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#E7D7E8] flex-shrink-0">
          <h2 className="text-lg font-bold text-[#2A1636]">
            {editingAssociate ? "✏️ Edit Associate" : "➕ Add Associate"}
          </h2>
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[#2A1636] mb-1.5">
                Associate Name <span className="text-red-400">*</span>
              </label>
              <input
                type="text"
                value={formData.name}
                ...
                placeholder="e.g. Odia Community Network"
              />
              ...
              {formData.logoPreview ? (
                <div className="relative w-24 h-24 rounded-xl overflow-hidden border-2 border-[#E7D7E8] bg-white">
                  <Image
                    ...
                    alt={formData.name || "Associate logo"}
                    width={96}
                    height={96}
              ...
              >
                {isSaving ? (
                  <span className="flex items-center justify-center gap-2">
                    ...
                    {editingAssociate ? "Updating..." : "Creating..."}
                  </span>
                ) : (
                  editingAssociate ? "Update Associate" : "Create Associate"
                )}
              </button>
            </div>
          </form>
