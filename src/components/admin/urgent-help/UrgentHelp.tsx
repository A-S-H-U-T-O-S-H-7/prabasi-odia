...
import { Check, Edit3, Eye, Loader2, RefreshCw, Trash2, X } from 'lucide-react';
    ...
    <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">{(['pending', 'approved', 'rejected', 'resolved'] as const).map(value => <button key={value} aria-pressed={status === value} onClick={() => setStatus(value)} className={`rounded-2xl border bg-white p-5 text-left ${status === value ? 'border-[#B45337]' : 'border-[#E7D7E8]'}`}><span className="text-xs capitalize text-[#6B5E5A]">{value === 'pending' ? 'Pending review' : value}</span><strong className="mt-2 block text-2xl text-[#2A1636]">{items.filter(item => item.status === value).length}</strong></button>)}</div>
      <div className="mb-5">
        <AdminSearchFilters
          ...
          isSearching={loading}
          placeholder="Search requests, locations or posters"
        >
          <select aria-label="Filter by status" value={status} onChange={event => setStatus(event.target.value as UrgentHelpStatus | 'all')} className="rounded-lg border border-[#E7D7E8] bg-white px-4 py-2 text-sm"><option value="all">All statuses</option><option value="pending">Pending review</option><option value="approved">Approved</option><option value="rejected">Rejected</option><option value="resolved">Resolved</option></select>
        </AdminSearchFilters>
      </div>
